...
class PerceptualMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        for modality, dim in [("text", self.vector_dim),
                              ("image", int(self._image_dim or self.vector_dim)),
                              ("audio", int(self._audio_dim or self.vector_dim))]:
            try:
                self.vector_stores[modality] = QdrantVectorStore(
                    url=os.getenv("QDRANT_URL"),
                    api_key=os.getenv("QDRANT_API_KEY"),
                )
            except Exception:
                self.vector_stores[modality] = None
