...
class EpisodicMemory(BaseMemory):
    ...
    def retrieve(self, query: str, limit: int = 5, **kwargs) -> List[MemoryItem]:
        ...
        # 若向量检索无结果，回退到关键词匹配（内存缓存 + SQLite）
        if not results:
            query_lower = query.lower()
            # 先从 SQLite 加载当前会话的历史 episodes 到内存
            if not self.episodes:
                try:
                    docs = self.doc_store.search_memories(
                        user_id=user_id, memory_type="episodic",
                    ...
                    for d in docs:
                        episode = Episode(
                            episode_id=d["memory_id"], user_id=d["user_id"],
                            session_id=d.get("properties", {}).get("session_id", ""),
                            timestamp=datetime.fromtimestamp(d["timestamp"]),
                            content=d["content"],
                            context=d.get("properties", {}),
