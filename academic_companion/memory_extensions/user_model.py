...
class UserModel:
    ...
    def get_summary(self) -> str:
        ...
        lines = [
            f"已学习 {total} 个主题，平均掌握度 {avg:.0f}%",
        ]
        if strong:
            lines.append("掌握较好的: " + ", ".join(f"{t}({m:.0f}%)" for t, m in strong))
        if weak:
            lines.append("需要加强的: " + ", ".join(f"{t}({m:.0f}%)" for t, m in weak))
        if due:
            lines.append("建议复习: " + ", ".join(t for t, _ in due))
