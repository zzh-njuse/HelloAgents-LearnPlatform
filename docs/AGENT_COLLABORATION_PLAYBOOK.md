...
Stage 2 已收尾，当前只进行 Stage 3 的文档准备：

- 从 Stage 2 总结、正确仓库代码和 `academic_companion` 资产中完成事实盘点。
- 起草并人工确认 course/section/lesson 事实模型、发布/重生成、Tutor runtime、session/memory、tool 权限和 eval 的 Spec/ADR。
- 开始实现前再次人工 Gate；不得因 Stage 2 已经有带引用回答而隐式引入 Agent、memory、session 或课程 schema。
