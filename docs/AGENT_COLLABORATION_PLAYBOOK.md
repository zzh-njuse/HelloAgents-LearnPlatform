...
Stage 3 已于 2026-07-16 收尾。Stage 4 Slice 1 已于 2026-07-17 完成练习、作答与反馈闭环，并通过自动化、OCR 与人工 Chrome 主路径 Gate；当前进入 Slice 2 掌握度、复习队列与可管理 Memory 的 Spec/ADR 分析，尚未批准 Slice 2 实现：

- Slice 2 必须先读取 `SLICE_1_SUMMARY.md` 和 `SLICE_2_INPUTS.md`，再盘点既有 practice/memory prototype；Spec/ADR 人工 Gate 通过前不得实现掌握度、复习队列或长期 Memory。
- Slice 1 的 Attempt/Feedback 只是候选学习事件输入，不得把单次错误、单次满分或简答 AI score 直接固化为长期用户画像。
- 删除类人工 smoke 经人工确认统一延后到 Stage 4 最终 Gate；各 Slice 的自动化删除测试仍必须保留。
- 真实 provider、OCR、人工 smoke、阶段性大提交和 push 仍由各自 Gate 控制。
- 不借 Slice 2 引入 Skill、MCP、自主多 Agent、认证、多租户、金额账单或完整运维 dashboard。
