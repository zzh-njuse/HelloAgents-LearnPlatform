...
Stage 3 已于 2026-07-16 收尾。Stage 4 Slice 1/2/3 已分别完成练习反馈、掌握度与 Memory、版本化教学 Skill，并通过各自自动化、OCR 与人工 Chrome 主路径 Gate；当前进入 Slice 4 单一 MCP 外部工具闭环的事实盘点与 Spec/ADR 准备，尚未选择或批准具体工具：

- Slice 4 必须先读取 `SLICE_3_SUMMARY.md` 和 `SLICE_4_INPUTS.md`，比较代码执行沙箱、数学工具和日历同步等候选方向；Spec/ADR 人工 Gate 通过前不得实现 MCP。
- 只选择一个不能由现有内部能力合理替代的学习场景；不得把 Postgres、Qdrant、Redis 或内部文件存储包装成 MCP，也不得开放任意命令、网络或网页搜索。
...
- [Stage 4 Slice 4 输入](./04-platform-stage-4-practice-memory-and-review/SLICE_4_INPUTS.md)
