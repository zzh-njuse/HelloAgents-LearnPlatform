...
五个 Slice 按事实依赖顺序推进。前一 Slice 只提供后一 Slice 可依赖的产品事实，不以一次性 prompt、隐藏状态或外部工具结果替代 Postgres 权威合同。Slice 5 是 2026-07-23 根据真实 smoke 新增的稳定性切片，不改变前四个 Slice 的能力边界。
...
## Slice 4：科学内容与受控 MCP 教学闭环（已完成）

目标：使用两项经过管理员审核的 capability 支持真实学习路径：自托管代码执行用于实验、编程题与 Tutor 讲解；远程 Wolfram 用于课程/练习科学验证与 Tutor 计算。公式显示由本地 KaTeX + `mhchem` 完成，不把渲染错误归给 MCP。

已选能力：

- Python/Java/C++ 隔离代码执行；
- Wolfram 数学、物理与通用化学计算；
- 本地受限公式渲染。

2026-07-21 修订 Spec/ADR 必须回答：
...
完成 Gate：公式在全部学习表面可靠显示；三语言编程题可执行并确定性评分；Lesson/Practice/Tutor 只在对应授权与预算内使用 Wolfram/代码；失败不伪造运行、计算或分数，且学习事实、外发、删除和隔离通过自动化、OCR 与人工安全验收。

明确不做：把内部 Postgres/Qdrant/Redis 操作包装成 MCP；接入第三项未经 Gate 的 capability；默认开放网页搜索、任意命令执行或无边界外发；让 Tool observation 直接成为学习事实。

## Slice 5：练习生成与评分链路稳定化（新增）

目标：在不新增能力的前提下，系统性稳定普通题、Python/Java/C++ 编程题和科学题的生成、reference validation、提交、确定性评分、教学反馈、去重与错误投影，并完成 Stage 4 最终交付 Gate。

新增原因：Slice 4 的真实 MCP 与安全边界已经通过验证，但人工 smoke 证明“工具可用”不等于“练习 artifact 稳定可用”。将生成质量继续塞入 MCP Slice 会混淆责任边界，因此独立收敛。

输入与边界见 [Slice 5 输入](SLICE_5_INPUTS.md)。该 Slice 必须重新完成事实盘点、Spec、必要 ADR 和人工 Gate；禁止针对固定课程、题干或关键词硬编码，也不得新增 MCP capability、修改 Memory/掌握度目标或引入自主多 Agent。
...
- Exercise Agent、Review Coach 或多 Agent 只在职责、artifact、重试/取消和成本边界真正可分时另行决策，不是五切片默认前提。
