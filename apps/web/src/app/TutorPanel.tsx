...
import { cancelTutorTurn, CourseReader, createTutorSession, createTutorTurn, deleteTutorSession, fetchLearningMemories, fetchLessonCompletions, fetchMemoryPolicy, fetchTutorSession, fetchTutorSessions, retryTutorTurn, TutorSession, tutorTurnEventsUrl } from "../lib/api";
    ...
    try {
      ...
      if (!current) {
        const accepted = window.confirm(memoryEnabled
          ? "问题、必要的课程资料片段，以及启用的学习记忆和课节完成摘要将发送给当前配置的外部 AI。是否继续？"
          : "问题和必要的课程资料片段将发送给当前配置的外部 AI。是否继续？");
        if (!accepted) return;
